...
The repository for the syslog2 project is on GitHub:

https://github.com/andy-maier/syslog2
...
1. If you have write access to the Git repo of this project, clone it using
   ...
   .. code-block:: bash
        ...
        $ git clone git@github.com:andy-maier/syslog2.git
        $ cd syslog2
...
project package at https://syslog2.readthedocs.io/
...
Python environment.
Depending on how the `syslog2` package is installed in
...
.. code-block:: bash
    ...
    $ pip download --no-deps --no-binary :all: syslog2==1.0.0
    $ tar -xf syslog2-1.0.0.tar.gz
    $ cd syslog2-1.0.0
    $ ls -1
    ...
    drwxr-xr-x  29 johndoe  staff    928 Jul  3 07:55 syslog2
    drwxr-xr-x   8 johndoe  staff    256 Jul  3 07:55 syslog2.egg-info
...
This section describes how to release a version of syslog2
to PyPI.
...
12. When releasing based on the master branch, activate the new version
    ...
    * Go to https://readthedocs.org/projects/syslog2/versions/
      and log in.
...
14. Upload the package to PyPI:
    ...
    **Attention!** This only works once for each version. You cannot release
    ...
    https://pypi.python.org/pypi/syslog2/
...
syslog2 project in its Git repo.
